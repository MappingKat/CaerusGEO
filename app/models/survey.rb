class Survey < ActiveRecord::Base
  ...
  def produceVectorLayersforResultPDF(entities)
    layers = []
    ...
    addLabelOffset = (map_form=='geo_point')

    type_to_produce = map_form

    entities.each_with_index do |entity,index|
      name = "marker_label_#{index}"
      styles[name] = {
                    :label => "#{(index+1)}",
                    :fontColor => "#ffffff",
                    :fontSize => 12
      ...
      if addLabelOffset
        styles[name][:labelYOffset] = 2
      end

      case type_to_produce
        when "geo_point"

          feats =  [{
                        :type => "Feature",
                        :id => "homeback#{index}",
                        :properties => {:_style => "homeback"},
                        :geometry => {:type => "Point",:coordinates =>
                            XGEOServices.GetPointInM([entity.points.first.lat,entity.points.first.lng])
                        }
                    }]
          labels =   [{
                        :type => "Feature",
                        :id => "markerlabel_label_#{index}",
                        :properties => {:_style => "marker_label_#{index}"},
                        :geometry => {:type => "Point",:coordinates =>
                            XGEOServices.GetPointInM([entity.points.first.lat,entity.points.first.lng])
                        }
                    }]
        ...
        when "geo_line"
         feats =   [{
                        ...
                        :geometry => {:type => "LineString",:coordinates =>
                            XGEOServices.GetLineString(entity.points)
                        }
                    }]
        labels =  entity.points.map do |coords|
          thishash = {
                            :type => "Feature",
                            :id => "markerlabel_label_#{index}",
                            :properties => {:_style => "marker_label_#{index}"},
                            :geometry => {:type => "Point",:coordinates =>
                                XGEOServices.GetPointInM([coords.lat,coords.lng])
                            }
                        }
          thishash
        ...
        when "geo_polygon" 
          feats = [     {
                        ...
                        :geometry => {:type => "Polygon",:coordinates => [
                            XGEOServices.GetFeatFromPointsInM(entity.points)
                        ]}
                    }
                  ]
          ...
          labels = [          {
                        ...
                        :properties => {:_style => "marker_label_#{index}"},
                        :geometry => {:type => "Point",:coordinates =>
                            XGEOServices.GetAreaCentroidInM(entity.points)
                        }
                    }
                  ]
          end
